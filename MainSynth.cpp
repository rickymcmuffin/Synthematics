...
MainSynth::MainSynth()
  ...
  if (!hasStarted) {
    // allASTs = EqAST("sin(f*2*3.14*x)",8);
...
//==============================================================================
const juce::String MainSynth::getName() const { return PLUGIN_NAME; }
...
double MainSynth::getTailLengthSeconds() const { return 0.0; }
...
int MainSynth::getCurrentProgram() { return 0; }

void MainSynth::setCurrentProgram(int index) { juce::ignoreUnused(index); }
...
void MainSynth::processBlock(juce::AudioBuffer<float> &buffer,
  ...
  // Now pass any incoming midi messages to our keyboard state object, and let
  // it add messages to the buffer if the user is clicking on the on-screen keys
