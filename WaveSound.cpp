...
WaveVoice::WaveVoice(std::shared_ptr<EqAST> aAST) { allASTs = aAST; }
...
void WaveVoice::stopNote(float /*velocity*/, bool allowTailOff) {
  if (allowTailOff) {
    ...
    if (tailOff ==
        0.0) // we only need to begin a tail-off if it's not already doing so -
...
void WaveVoice::renderNextBlock(juce::AudioBuffer<float> &outputBuffer,
                                int startSample, int numSamples) {
  if (xDelta != 0.0) {
    while (--numSamples >= 0) {
      auto currentSample = (float)allASTs->getResult(xCurrent, frequency);
